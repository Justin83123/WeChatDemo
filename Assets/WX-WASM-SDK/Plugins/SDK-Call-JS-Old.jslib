    ...
    glCompressedTexImage2D: function (target, level, internalFormat, width, height, border, imageSize, data) {
        ...
        function getMatchId() {
            if(GameGlobal.USED_TEXTURE_COMPRESSION && internalFormat == 36196){
                ...
                var start = res.length - 8;
                var header = [ '_', 'a', 's', 't', 'c'];
                for (var i = 0;i < header.length;i++){
                    if(res[start + i] != header[i]){
                        return [ matchId, '8x8'];
                    }
                }
                var astcBlockSize = matchId.substring(start + 5);
                return [ matchId.substr(0,start) , astcBlockSize ];
            }
            return [-1 ,'8x8'];
        }
            ...
            switch (compressFormat) {
                case "astc":
                    ...
                    if(astcBlockSize == '6x6'){
                        format = GLctx.getExtension("WEBGL_compressed_texture_astc").COMPRESSED_RGBA_ASTC_6x6_KHR;
                        break;
                    }
        ...
        if (matchId != -1) {
            ...
            switch (GameGlobal.TextureCompressedFormat) {
                case "astc":
                    ...
                    if(astcBlockSize == '6x6'){
                        f = GLctx.getExtension("WEBGL_compressed_texture_astc").COMPRESSED_RGBA_ASTC_6x6_KHR;
                        break;
                    }
